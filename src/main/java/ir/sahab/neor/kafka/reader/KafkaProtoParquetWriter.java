...
/**
 * Instance of this class reads proto objects from a Kafka topic with SmartCommitKafkaConsumer
 * and writes them on parquet files.
...
public class KafkaProtoParquetWriter<T extends Message> implements Closeable {
    ...
    private KafkaProtoParquetWriter(Builder<T> builder) {
        ...
        kafkaConfig.putAll(builder.consumerConfig);
        kafkaConfig.put(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, false);
        kafkaConfig.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, ByteArrayDeserializer.class);
        kafkaConfig.computeIfAbsent(ConsumerConfig.GROUP_ID_CONFIG, key -> "KafkaProtoParquetWriter-" + instanceName);
        ...
        smartCommitKafkaConsumer = new SmartCommitKafkaConsumer<>(kafkaConfig,
                builder.offsetTrackerPageSize,
                builder.offsetTrackerMaxOpenPagesPerPartition,
                builder.maxQueuedFiles);
        smartCommitKafkaConsumer.subscribe(topic);
    }

    /**
     * Shard writers starts to listen to configured SmartCommitKafkaConsumer and writing received messages to
     * parquet files.
     *
    ...
    /**
     * Closes all parquet files and SmartCommitKafkaConsumer. <b>Note that this method does not throw
            ...
            if (!writtenOffsets.isEmpty()) {
                writtenOffsets.forEach(partitionOffset -> {
                    try {
                        ...
                        logger.info("Kafka producer callback interrupted.");
    ...
    public static class Builder<T extends Message> {
        ...
        private final int maxQueuedFiles;
         ...
         * @param offsetTrackerPageSize read about it in SmartCommitKafkaConsumer Class
         * @param offsetTrackerMaxOpenPagesPerPartition read about it in SmartCommitKafkaConsumer Class
         * @param maxQueuedFiles read about it in SmartCommitKafkaConsumer Class
         */
        public Builder(String instanceName, String topic, Map<String, Object> consumerConfig, int offsetTrackerPageSize,
                int offsetTrackerMaxOpenPagesPerPartition, int maxQueuedFiles, String targetDir, Class<T> protoClass, Parser<T> parser) {
