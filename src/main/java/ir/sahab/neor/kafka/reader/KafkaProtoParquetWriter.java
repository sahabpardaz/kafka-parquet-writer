 ...
 * can not be done concurrently, each threads writes to a separate file.
 ...
 * Currently these policies are supported for closing a file and opening a new one:<br>
 *  - File Size: When size of the parquet file reaches a threshold<br>
 *  - Open Time: When a file has been open for certain amount of time<br>
 * whenever any of configured criteria has been met. Names of finalized files will be
 ...
 * written in a parquet file and successfully flushed to the disk.
 *
...
public class KafkaProtoParquetWriter<T extends Message> implements Closeable {
    ...
    private static Histogram fileSize;
    ...
    private KafkaProtoParquetWriter(Builder<T> builder) {
        ...
        if (metricRegistry != null) {
            ...
            fileSize = metricRegistry.histogram("parquet.writer.average.file.size");
        }
    ...
    /**
     * Reading from SmartCommitKafkaConsumer and Writing to parquet files are done using instances of this class.
    ...
    private class WorkerThread implements Runnable, Closeable {
        ...
        /**
         * Starts to listen to configured SmartCommitKafkaConsumer and Writing received messages to parquet file
        ...
        private void finalizeCurrentFile() throws InterruptedException {
            ...
            if (!writtenOffsets.isEmpty()) {
                ...
                logger.debug("Committing {} offsets for '{}' file.", writtenOffsets.size(), finalFilePath);
    ...
    public static class Builder<T extends Message> {
        ...
        private final int offsetTrackerPageSize;
        private final int offsetTrackerMaxOpenPagesPerPartition;
        private final int maxFileOpenDurationSeconds;
        private final String targetDir;
        private final Class<T> protoClass;
        private final Parser<T> parser;

        private int maxQueuedRecordsInConsumer = 100 * 1000;
         ...
         * @param offsetTrackerPageSize is used in SmartCommitKafkaConsumer, the size of each page in offset tracker.
         ...
         * @param offsetTrackerMaxOpenPagesPerPartition is used in SmartCommitKafkaConsumer, maximum number of open
         ...
         * @param maxFileOpenDurationSeconds maximum time each file is kept open in milliseconds, after this
         ...
         */
        public Builder(String instanceName, String topic, Map<String, Object> consumerConfig, int offsetTrackerPageSize,
                int offsetTrackerMaxOpenPagesPerPartition,int maxFileOpenDurationSeconds,
                int maxExpectedThroughputPerSecond, String targetDir, Class<T> protoClass, Parser<T> parser) {
            ...
            Validate.isTrue(offsetTrackerPageSize > 0, "offsetTrackerPageSize of smart consumer must be positive.");
            Validate.isTrue(offsetTrackerMaxOpenPagesPerPartition > 0,
                    "offsetTrackerMaxOpenPagesPerPartition of smart consumer must be positive.");
            Validate.isTrue(maxFileOpenDurationSeconds > 0, "Maximum duration of file must be positive.");
            Validate.isTrue(maxExpectedThroughputPerSecond > 0, "maxExpectedThroughputPerSecond must be positive.");
            Validate.isTrue(offsetTrackerPageSize * offsetTrackerMaxOpenPagesPerPartition >
                            1.3 * maxExpectedThroughputPerSecond * maxFileOpenDurationSeconds,
                    "\"offsetTrackerPageSize * offsetTrackerMaxOpenPagesPerPartition > "
                            + "1.3 * maxExpectedThroughputPerSecond * maxFileOpenDurationSeconds\""
                            + "\n above expression must be satisfied, because if is not we have temporary deadlock.");
        ...
        /**
         * @param maxQueuedRecordsInConsumer is used in SmartCommitKafkaConsumer, maximum number of records
        ...
        public Builder<T> maxQueuedRecordsInConsumer(int maxQueuedRecordsInConsumer) {
            Validate.isTrue(maxQueuedRecordsInConsumer > 0, "Maximum duration of file must be positive.");
